...
The plugin imports into a Cursor team marketplace and installs in Grok Bot with a working EU/US region selector. Both EU and US completed OAuth with owner-approved permissions and passed read-only `list_workflows` calls. On September 24, after the workflow-query fix reached production, the EU demo query returned one row (identifier only); the US connection passed a lightweight GraphQL query. The earlier prerequisite error did not recur. Existing connections were reused without another login, and EU was restored after testing. No proposed backend callback change was deployed. Write operations, token-expiry refresh, and public-marketplace installation remain unverified. An earlier Cursor local registration failure does not describe the verified Grok Bot flow. See [TESTING.md](TESTING.md) for evidence and remaining checks.
...
One plugin supports either endpoint through the required `V7_GO_MCP_URL` configuration variable. No default region is assumed. The two allowed values are declared using JSON Schema `enum`, as supported by the [Cursor plugin reference](https://cursor.com/docs/reference/plugins#variables). The region dropdown has been verified in both the dashboard and Grok Bot. Grok Bot asks for the region during its own installation flow. Both regional selections have completed OAuth and a read-only workflow-list test in Grok Bot. Bounded EU workflow querying and lightweight US GraphQL execution also passed after the September 24 production fix.
...
Version 0.1.1 temporarily split the endpoints into two plugin entries to isolate an OAuth failure during local testing. Version 0.1.2 uses one plugin with a required region choice, following Cursor's documented dashboard variable configuration. The earlier local substitution failure does not demonstrate that marketplace variables are unsupported. Team-marketplace import, Grok Bot region selection, installation, authorization and read-only workflow-list calls in both EU and US have been verified. EU connection reuse after restoring the region and reopening the app also passed. Workflow querying passed its September 24 production retest after the prerequisite fix. Write operations, token-expiry refresh, and public-marketplace distribution remain unverified.
