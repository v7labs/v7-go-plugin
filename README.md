...
One plugin supports either endpoint through the required `V7_GO_MCP_URL` configuration variable. No default region is assumed. The two allowed values are declared using JSON Schema `enum`, as supported by the [Cursor plugin reference](https://cursor.com/docs/reference/plugins#variables). The region dropdown has been verified in both the dashboard and Grok Bot. Grok Bot asks for the region during its own installation flow. Both regional selections have completed OAuth and a read-only workflow-list test in Grok Bot. The workflow-result query limitation below remains unresolved.
